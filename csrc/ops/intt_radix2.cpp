...
void intt_radix2(std::vector<torch::Tensor> a,
                 ...
                 const std::vector<torch::Tensor> ipsi,
                 const int64_t sp_prime_len) {
  const auto num_devices = a.size();
  for (size_t i = 0; i < num_devices; ++i) {
    intt_radix2_cuda(a[i], even[i], odd[i], ipsi[i], sp_prime_len);
  }
}
...
void intt_radix2_exit(std::vector<torch::Tensor> a,
  ...
  const auto num_devices = a.size();
  for (size_t i = 0; i < num_devices; ++i) {
    intt_radix2_exit_cuda(a[i], even[i], odd[i], ipsi[i], sp_prime_len);
  }
}
...
void intt_radix2_exit_reduce(std::vector<torch::Tensor> a,
  ...
  const auto num_devices = a.size();
  for (size_t i = 0; i < num_devices; ++i) {
    intt_radix2_exit_reduce_cuda(a[i], even[i], odd[i], ipsi[i], sp_prime_len);
  }
}
