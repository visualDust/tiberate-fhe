...
//     const std::vector<torch::Tensor> _2q) {
//   std::vector<torch::Tensor> outputs;
//   const auto num_devices = in.size();
//   for (size_t i = 0; i < num_devices; ++i) {
//     auto out =
//         codec_rotate_make_unsigned_reduce_2q_cuda(in[i], perm[i], _2q[i]);
//     outputs.push_back(out);
//   }
//   return outputs;
// }
...
TORCH_LIBRARY_FRAGMENT(tiberate_fused_ops, m) {
  ...
  m.def(
      ...
      "Tensor[] kl, Tensor[] kh) -> ()");
  // m.def(
  //     "codec_rotate_make_unsigned_reduce_2q(Tensor[] in, "
  //     "Tensor[] perm, Tensor[] _2q) -> Tensor[]");
}
