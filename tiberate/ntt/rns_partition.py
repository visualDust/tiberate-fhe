...
class RnsPartition:
    ...
    def compute_destination_arrays(self):
        filter_alloc = lambda devi, i: [a for a in self.flat_prime_allocations[devi] if a >= i]
    ...
    def compute_partitions(self):
        ...
        self.d_special = [
            self.destination_arrays_with_special[0][dev_i] for dev_i in range(self.num_devices)
        ]
        ...
        for lvl in range(self.num_ordinary_primes):
            ...
            destp_special = [[[d[pi] for pi in p] for p in dev_p] for d, dev_p in zip(dest, par)]
            ...
            p_special = [
                [[pi + d for pi in p] for p in dev_p] for d, dev_p in zip(diff, self.parts[lvl])
            ]
